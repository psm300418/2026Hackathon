...
제품 카테고리, 항목 유형, 피부 상태 태그, 생활·환경 요인 태그에 사용한다.
...
- 현재 제품 이미지를 추가할 계획이 없으므로 카드의 시각 중심은 브랜드, 카테고리, 제품명, 대표 전성분으로 둔다.
- 제품명은 15sp, 700으로 2줄까지 허용한다.
- 설명은 12sp 또는 13sp로 2줄까지 제한한다.
- 제품 등록 상태는 작은 배지로 표시한다.
- 항목 유형이 필요한 화면에서는 화장품, 샤워용품, 영양제를 작은 텍스트 배지로 구분한다.
...
- 추출 완료 후에는 사용자가 검토할 텍스트와 성분·원료 후보를 중심으로 보여준다.
...
- 배경: `Surface`
- 상단 검색창
- 카테고리, 항목 유형, 상태 필터는 가로 스크롤 칩으로 구성한다.
- 현재 제품 이미지를 추가할 계획이 없으므로 제품 카드는 이미지 없이 브랜드, 카테고리, 제품명, 대표 전성분 중심으로 배치한다.
- 이전 사용 제품 등록은 설정 탭에서 제공한다.
- Android 카드의 대표 전성분은 `정제수`를 제외하고 앞쪽 성분 몇 개만 표시한다.
- T3 범위에서는 등록된 제품이 없거나 검색 결과가 없어도 직접 등록 CTA를 제공하지 않는다.

### 9.3 제품 등록 및 AI 추출 검토

- 첫 화면은 항목 유형, 제품명, 브랜드, 카테고리 입력과 성분표 또는 라벨 사진 제출에 집중한다.
- 항목 유형은 화장품, 샤워용품, 영양제로 구분한다.
- AI 추출 결과 화면은 원문 텍스트, 표준화된 성분·원료 후보, 매칭 실패 항목을 분리해 보여준다.
- 사용자가 직접 수정하고 최종 확인해야 저장된다.
- 저장 CTA는 화면 하단에 고정한다.

### 9.4 기록 화면

- 오늘 기록은 하루 1개이며, 피부 상태 입력이 화면의 중심이다.
- 건조함, 유분, 붉음, 트러블은 0부터 5까지의 stepper 또는 segmented control로 빠르게 입력한다.
- 수면 시간은 숫자 입력 또는 stepper로 입력하며 0부터 24까지 허용한다.
- 외출 시간은 분 단위 stepper 또는 빠른 선택 칩으로 입력한다.
- 지역이 설정되어 있으면 날씨, 온도, 습도, 강수 상태를 작은 환경 요약 영역으로 표시한다.
- 지역이 없으면 설정 탭으로 이동할 수 있는 짧은 empty state를 제공한다.
- 오늘 사용 제품은 여러 개 선택할 수 있게 칩 또는 체크 리스트로 표시한다.
- 제품 선택 영역은 필요하면 화장품, 샤워용품, 영양제 섹션으로 나눈다.
- 제품 프리셋은 상단의 빠른 적용 칩으로 제공하고, 적용하면 포함 제품들이 선택 상태가 된다.
- 기록 화면 안에서 공용 제품 DB 검색 후 내 제품 목록에 추가하고 바로 오늘 사용 제품으로 선택할 수 있다.
- 얼굴 사진 업로드 영역은 보조 옵션으로 배치하며 사진 없이도 저장 버튼을 사용할 수 있어야 한다.
- 스트레스 입력은 T4 범위에서 제외한다.

### 9.5 기록 히스토리와 추이 화면

- 캘린더 또는 날짜 리스트로 기록 여부를 보여준다.
- 날짜 상세에는 피부 상태 점수, 사용 제품, 수면, 외출, 날씨 요약, 선택 사진을 함께 보여준다.
- 추이 화면은 기간 선택을 먼저 제공하고, 건조함, 유분, 붉음, 트러블 변화를 같은 척도로 비교한다.
- 날씨와 수면은 그래프를 압도하지 않도록 작은 요약 배지나 보조 행으로 표시한다.
...
- 수면, 외출, 날씨, 습도, 온도 같은 환경 요인은 제품·성분 후보의 보조 근거로만 표시한다.
...
| Settings | settings 또는 user |
| Save | check |
| Add | plus |
| Favorite | heart |
| Weather | cloud-sun |
| Location | map-pin |
| Notification | bell. 알림은 추가 기능 화면에서만 사용 |
...
현재 MVP 구현에서는 `SkinColors`, `SkinSpacing`, `SkinRadius`와 공통 `SectionHeader`, `InfoCard`, `StatusText`를 먼저 제공하고, 화면별 상세 컴포넌트는 기능 안정화 후 확장한다.
