...
> 문서 상태: 초안
> 목적: 해커톤 시연용 계정, 제품, 프리셋, 최근 90일 기록, 과거 분석 기록 준비
...
실행일 기준 최근 90일의 `daily_records`를 생성한다. `2026-08-19`에 실행하면 `2026-05-22`부터 `2026-08-19`까지다.

기록에는 다음 값을 현실적인 범위로 넣는다.

- 피부 항목 점수: 건조함, 유분, 붉음, 트러블을 0-5 범위로 기록
- 생활 변수: 수면 시간, 외출 시간
- 사용 제품: 아침 기본 루틴은 대부분 매일, 저녁 회복 루틴은 주 4-5회, 폼/젤 클렌저는 1주일 단위로 교체하며 90일 동안 반복, 두피/영양 관리는 격일 또는 주기적으로 반영
- 환경 스냅샷: 서울특별시 강남구 기준의 데모 온도, 습도, 강수량, 풍속
- 메모: 수면 부족, 외출, 보습, 두피 관리 같은 짧은 사용자 메모

분석 시연이 밋밋하지 않도록 아래 포인트를 의도적으로 포함한다.

- 각 30일 구간 중간 지점에 `글로우 리치 나이트 크림` 사용일을 만들고, 같은 날 수면 부족, 긴 외출, 높은 습도를 함께 기록한다.
- 2주차에는 `AC 컬렉션 카밍 폼 클렌저` 사용과 함께 트러블·붉음이 내려가는 흐름을 만들어 `Salicylic Acid`, `Asiaticoside`, `Madecassic Acid` 같은 특징 성분이 긍정 후보로 잡히게 한다.
- 4주차에는 `레드 라이스 이노시톨 포어 클라리파잉 딥 클렌저` 사용과 낮은 습도·건조함 증가를 겹치게 해 `Kaolin`, `Hydrated Silica`, `Cellulose` 같은 성분이 부정 후보로 잡히게 한다.
...
최근 30일 이전의 원본 기록을 매번 AI에 모두 보내지 않는 구조를 시연하기 위해, seed 스크립트는 최신 분석을 실행하기 전에 과거 분석 run 2개를 생성한다.

- 60일 전 분석: 초기 30일 구간의 BHA 클렌저 긍정 후보와 수면·습도 악화 후보를 요약한다.
- 30일 전 분석: 두 번째 30일 구간의 반복 패턴과 포어 딥클렌징 주간의 건조함 증가 후보를 요약한다.
- 최신 분석은 최근 30일 상세 기록, 90일 전체 압축 성분 통계, 직전 분석 요약을 함께 사용한다.
...
- 분석 실행 결과: seed 스크립트가 과거 분석 2개와 최신 분석 1개를 생성하므로 분석 탭 진입 시 이전 맥락이 반영된 분석 결과를 바로 보여줄 수 있다.
