...
- 작업:
  - 기록 탭에서 오늘 기록 작성
  - 피부 상태 점수는 건조함, 유분, 붉음, 트러블별로 0부터 5까지의 선택 칸으로 입력
  - 수면 시간 입력
...
- 작업:
  ...
  - 추이는 최근 30일 고정으로 표시
  - 피부 상태 변화 요약, 평균, 변화량 표시
  - 건조함, 유분, 붉음, 트러블별 라인 그래프 카드 표시
  - 사용 제품과 환경 요인 요약
- 완료 조건:
  - 사용자가 최근 1달의 피부 상태 변화를 한눈에 확인 가능
  - 7일/14일 기간 선택은 제공하지 않음
  - 자료가 부족하면 실제 있는 기록만 그래프로 표시
...
- 작업:
  ...
  - 최근 평균 대비 피부 컨디션 점수가 낮아진 날짜를 특이 변화 이벤트로 계산
  - 특이 변화 이벤트에 수면 부족, 높은 습도, 긴 외출, 첫 사용 제품 같은 후보 요인 연결
  - Android 시각화를 위한 최근 30일 `trendPoints`, `notableEvents`, `factorSummaries` 생성
- 완료 조건:
...
- 작업:
  - 분석 탭 진입 시 최근 분석 조회
  - 분석 요청 버튼
  - 최근 30일 피부 컨디션 점수 시각화
  - 평소 대비 컨디션 점수가 낮아진 날짜와 후보 요인 카드
  - 긍정적 의심 성분 카드
  - 부정적 의심 성분 카드
  - 근거 수준 배지
  - 데이터 부족 안내
